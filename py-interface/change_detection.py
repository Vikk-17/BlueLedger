...
class ChangeDetectionAnalyzer:
    ...
    def get_baseline_period(self, current_date: datetime, months_back: int = 12) -> Tuple[str, str]:
    ...
    def calculate_ndvi_change(
        ...
        valid_mask = (
            ~np.isnan(current_ndvi) &
            ~np.isnan(baseline_ndvi)
        )
        ...
        logger.info(f"Change: {results['trend']}, Improved: {results['percent_improved']:.1f}%")
        return results
