...
class CarbonCalculator:
    ...
    def calculate_pixel_area(
        ...
        if crs is None or not crs.is_projected:
            raise ValueError(
                "Pixel area calculation requires a projected CRS (UTM)."
            )
    ...
    def calculate_from_rasters(
        ...
        with rasterio.open(ndvi_path) as ndvi_src, \
            ...
            pixel_area_ha = self.calculate_pixel_area(
                ndvi_src.transform,
                ndvi_src.crs
            )
            ...
            for idx, (type_name, model) in enumerate(priority_models):
                ...
                agb_array = np.array([
                    self.ndvi_to_agb(ndvi_val, model)
                    for ndvi_val in ndvi_type
                ])
            ...
            results = {
                ...
                'co2e_per_ha': float(total_carbon / total_area_ha) if total_area_ha > 0 else 0.0,
                'breakdown': breakdown,
    ...
    def calculate_from_means(
        ...
        model = self.biomass_models.get('moderate_forest', {
            'a': 120.3,
            'b': -35.4
        })
