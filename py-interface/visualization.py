...
class Visualizer:
    ...
    def create_forest_type_map(
        ...
        colors = ['#8B4513', '#228B22', '#90EE90', '#D3D3D3']  # brown, green, light green, gray
        cmap = LinearSegmentedColormap.from_list('forest', colors, N=4)
        
        im = ax.imshow(forest_type, cmap=cmap, vmin=0, vmax=3)
        
        # Create legend
        labels = [model.get('name', f'Type {i}') for i, model in enumerate(biomass_models.values())]
    ...
    def create_carbon_breakdown_chart(
        self,
        breakdown: Dict
    ) -> Path:
        """
